...
# print(torch.cuda.device_count())  # 应打印出大于1的数字，如果正确设置了多个GPU

def pcd_update(args, model, interpolated_pcd):
    # interpolated_pcd: (b, 3, n)    interpolated_pcd是插值生成的点

    pcd_pts_num = interpolated_pcd.shape[-1]
    # 1024
    patch_pts_num = args.num_points * 4
    # extract patch             #  patch_rate = 3
    ...
    # global_feats, local_feats = model.module.extract_feature(patches)
    ...
    for i in range(args.num_iterations):
        ...
        # 使用 .module 属性来正确调用 regress_distance 方法
        # pred_p2p = model.module.regress_distance(patches, updated_patch, global_feats, local_feats)
...
def pcd_upsample(args, model, input_pcd):
    ...
    # interpolated_pcd = midpoint_interpolate_v1(args, input_pcd)
    # update: (b, 3, m)
...
def test(args):
    ...
    for i, path in tqdm(enumerate(test_input_path), desc='Processing'):
        ...
        # up_pcd = int(upsampled_pcd.shape[-1] / 2)
        # upsampled_pcd = FPS(upsampled_pcd, up_pcd)
