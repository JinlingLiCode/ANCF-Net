...
# from models.FeatureExtractor_all import FeatureExtractor
# from models.FeatureExtractor_gcn import FeatureExtractor
# from models.FeatureExtractor_c2f import FeatureExtractor
from models.P2PRegressor import P2PRegressor
from models.utils import get_knn_pts, index_points
from models.interpolate_feature_weight import interpolate_feature_weight,interpolate_feature_point_Multi_scale_adaptive_Knn_attention
# from models.interpolate_feature_weight_local import interpolate_feature_weight,interpolate_feature_point_Multi_scale_adaptive_Knn_attention
# from models.interpolate_feature_weight_global import interpolate_feature_weight,interpolate_feature_point_Multi_scale_adaptive_Knn_attention
# from models.interpolate_feature_weight_sum import interpolate_feature_weight,interpolate_feature_point_Multi_scale_adaptive_Knn_attention,interpolate_feature_Dynamic_k
class P2PNet(nn.Module):
    def __init__(self, args):
        ...
        for i in range(args.block_num + 1):
            self.weights.append(interpolate_feature_point_Multi_scale_adaptive_Knn_attention(args))
    ...
    def interpolate_feature(self, original_pts, query_pts, local_feat):
        ...
        #求查询点对于原始点云的k近邻，对每个原始点对于k个近邻点距离计算，根据距离的倒数进行归一化加权，
        # 距离越近权重越大，将权重跟knn_feat相乘，最后得到每个查询点对于k近邻点的特征
        k = 3
    ...
    def regress_distance(self, original_pts, query_pts, global_feats, local_feats):
        ...
        # for i in self.weights :
            # (b, c, n)
            interpolated_feat = self.weights[i](original_pts, query_pts, local_feats[i])
            interpolated_local_feats.append(interpolated_feat)
        # (b, c*(block_num+1), n)
        agg_local_feats = torch.cat(interpolated_local_feats, dim=1)
        # (b, 3+c*(block_num+2), m)
